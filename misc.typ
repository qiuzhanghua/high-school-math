...
$ cos (A-B)/2 = 3 cos (A+B)/2. $ <eq:73641>
...
*3. 求定值。* 已知 $a + b + c = 12$，结合 @eq:73642 得 $4c = 12$，即 $c = 3$，从而 $a + b = 9$。
...
#v(1em)
// ==================== 第三题 ====================
#section-title("四、2022 年全国卷 理科第 20 题／文科第 21 题（圆锥曲线·定点问题）")

*题目：* 已知椭圆 $E$ 的中心为坐标原点，对称轴为 $x$ 轴、$y$ 轴，且过 $A(0, -2)$、$B(3/2, -1)$ 两点。

(1) 求 $E$ 的方程；

(2) 设过点 $P(1, -2)$ 的直线交椭圆 $E$ 于 $M$、$N$ 两点，过点 $M$ 且平行于 $x$ 轴的直线与线段 $A B$ 交于点 $T$，点 $H$ 满足 $arrow(M T) = arrow(T H)$。证明：直线 $H N$ 过定点。

*解析：*

*(1) 求方程。* 设 $E: x^2/m + y^2/n = 1$（$m, n > 0$）。代入 $A(0, -2)$ 得 $4/n = 1$，即 $n = 4$；再代入 $B(3/2, -1)$：

$ (9/4)/m + 1/4 = 1 quad => quad (9/4)/m = 3/4 quad => quad m = 3. $
...
*思路：* 设 $M(x_1, y_1)$、$N(x_2, y_2)$，证明 $k_(A H) = k_(A N)$——两斜率都以 $A$ 为公共端点，故 $A$、$H$、$N$ 三点共线，直线 $H N$ 恒过定点 $A(0, -2)$。

*① 直线 $A B$ 与点 $T$、$H$ 的坐标。* 由 $A(0, -2)$、$B(3/2, -1)$ 得

$ k_(A B) = ((-1) - (-2))/(3/2 - 0) = 2/3, quad A B: y = -2 + 2/3 x. $

设过 $P(1, -2)$ 的直线为 $y = k x - k - 2$，它与椭圆交于 $M(x_1, y_1)$、$N(x_2, y_2)$，则

$ y_i + 2 = k(x_i - 1) quad (i = 1, 2). quad (*) $

因为 $M T parallel x$ 轴，$T$ 与 $M$ 纵坐标相同，又 $T$ 在直线 $A B$ 上，所以

$ x_T = 3/2 (y_1 + 2). $

再由 $arrow(M T) = arrow(T H)$ 知 $T$ 是 $M H$ 的*中点*，于是

$ x_H = 2 x_T - x_1 = 3(y_1 + 2) - x_1, quad y_H = y_1. quad (**) $

*② 由"中点"直接得到一个倒数斜率之和。* 因为 $A(0,-2)$、$H$、$M$ 的斜率分别为

$ 1/k_(A M) = (x_1 - 0)/(y_1 + 2), quad 1/k_(A H) = (x_H - 0)/(y_H + 2) = x_H/(y_1 + 2), $

所以

$
  1/k_(A M) + 1/k_(A H) = (x_1 + x_H)/(y_1 + 2) = ((x_1) + (3(y_1+2) - x_1))/(y_1 + 2) = (3(y_1+2))/(y_1+2) = 3 = 2/k_(A B).
$

即

$ 1/k_(A M) + 1/k_(A H) = 2/k_(A B) = 3. $ <eq:73643>


*③ 用韦达定理直接算 $1/k_(A M) + 1/k_(A N)$。* 由 $y_i + 2 = k(x_i - 1)$ 得

$ 1/k_(A M) = (x_1 - 0)/(y_1 + 2) = x_1/(k(x_1 - 1)), quad 1/k_(A N) = x_2/(k(x_2 - 1)), $

通分：

$
  1/k_(A M) + 1/k_(A N) = 1/k dot (x_1(x_2 - 1) + x_2(x_1 - 1))/((x_1 - 1)(x_2 - 1)) = 1/k dot (2 x_1 x_2 - (x_1 + x_2))/(x_1 x_2 - (x_1 + x_2) + 1).
$

把 $y = k x - k - 2$ 代入 $x^2/3 + y^2/4 = 1$：

$ 4 x^2 + 3(k x - k - 2)^2 = 12 quad => quad (4 + 3k^2) x^2 - 6k(k+2) x + 3(k+2)^2 - 12 = 0, $

由韦达定理

$ x_1 + x_2 = (6k(k+2))/(4 + 3k^2), quad x_1 x_2 = (3(k+2)^2 - 12)/(4 + 3k^2). $ <eq:73644>

代入并化简：分子分母同乘 $4 + 3k^2$ 后分别展开，

$ (4 + 3k^2)[2 x_1 x_2 - (x_1 + x_2)] = 2[3(k+2)^2 - 12] - 6k(k+2) = 6k^2 + 12k = 12k, $

$ (4 + 3k^2)[x_1 x_2 - (x_1 + x_2) + 1] = [3(k+2)^2 - 12] - 6k(k+2) + (4 + 3k^2) = 4, $

$($第一式：$6(k^2+4k) - 6k^2 - 12k = 12k$；第二式：$3k^2+12k - 6k^2-12k + 4+3k^2 = 4$。$)$

所以

$ 1/k_(A M) + 1/k_(A N) = 1/k dot (12k)/(4) = 1/k dot 3k = 3 quad ("与" k "无关"). $ <eq:73645>

*④ 两式相减，得 $k_(A H) = k_(A N)$。* 比较 @eq:73643 与 @eq:73645：

$ 1/k_(A M) + 1/k_(A H) = 3, quad 1/k_(A M) + 1/k_(A N) = 3, $

两式相减得

$ 1/k_(A H) = 1/k_(A N) quad => quad k_(A H) = k_(A N). $
